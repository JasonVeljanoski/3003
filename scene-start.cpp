...
float POSE_TIME = 0.0; // Part 2.
unsigned int animationPause = 0; // Part 2.
...
static void addObject(int id)
{
    ...
    // Part 2.
    sceneObjs[nObjects].animationStart = 0.0;
...
void init( void )
{
    ...
    // Part 2. [B] initialise variables vBoneIDs, vBoneWeights, uBoneTransforms
    ...
    // Part I. Define second light sphere
...
void drawMesh(SceneObject sceneObj)
{
    ...
    /* (Part B) my-imp */
    // Part B is also in fragment shader !
    // Set the model matrix. 
	mat4 rotM = RotateZ(sceneObj.angles[2]) * RotateY(sceneObj.angles[1]) * RotateX(-sceneObj.angles[0]);
        
	mat4 model = Translate(sceneObj.loc) * rotM * Scale(sceneObj.scale);
    /* end */
...
void display( void )
{
    ...
    /* (Part A) my-imp */
    // Set the view matrix.
	mat4 rotM = RotateX(camRotUpAndOverDeg) * RotateY(camRotSidewaysDeg); // add appropriate rotations.
	view = Translate(0.0, 0.0, -viewDist) * rotM;
    /* end */
   ...
   vec4 lightPosition2 = rotM * lightObj2.loc;
   glUniform4fv( glGetUniformLocation(shaderProgram, "LightPosition2"),1, lightPosition2); CheckError();

    for (int i=0; i < nObjects; i++) {
        SceneObject so = sceneObjs[i];
      
        vec3 rgb = so.rgb * lightObj1.rgb * so.brightness * lightObj1.brightness * 5.0;
	...
	glUniform1f( glGetUniformLocation(shaderProgram, "Shininess"), so.shine );
        ...
        CheckError();
        vec3 rgb2 = so.rgb * lightObj2.rgb * so.brightness * lightObj2.brightness * 5.0;  // Part I.
      
        glUniform3fv( glGetUniformLocation(shaderProgram, "DiffuseProduct2"), 1, so.diffuse * rgb2 ); // Part I.
        glUniform3fv( glGetUniformLocation(shaderProgram, "SpecularProduct2"), 1, so.specular * rgb2 ); // Part I.
	glUniform3fv( glGetUniformLocation(shaderProgram, "AmbientProduct2"), 1, so.ambient * rgb2 ); // Part I.
	    ...
	    float elapsedTime = 0.0;
            ...
            if ( sceneObjs[i].FPC < 0.0 ) sceneObjs[i].FPC = 0.0; // avoid negative FPC
	    if ( sceneObjs[i].moveDist <= 0.0 ) sceneObjs[i].moveDist = 0.1; // avoid dividing by 0 - dirty
            if ( sceneObjs[i].moveSpeed <= 0.0 ) sceneObjs[i].moveSpeed = 0.1;

            if ( animationPause != 0 ) { // if animation is paused
                elapsedTime = float ( animationPause - sceneObjs[i].animationStart ) / 1000.0; // time since animation began at time of pause
            } else {
...
static void lightMenu(int id)
{
    ...
    // Part I. Link second light to light menu
    else if (id == 80) {
        toolObj = 2;
        setToolCallbacks(adjustLocXZ, camRotZ(),
                         adjustBrightnessY, mat2( 1.0, 0.0, 0.0, 10.0) );
...
static void materialMenu(int id)
{
    ...
    /* Part C. */
    /* id == 20: Ambient/Diffuse/Specular/Shine position*/
    else if (id == 20) {
      toolObj = currObject;
      /* "Recall that the shine value needs to be able to increase to about 100 or so" - project Part C.*/
      /* Are the values correct ? */
      setToolCallbacks( modifyAmbientDiffuse, mat2(1.0,0.0,0.0,1.0), modifySpecularShine, mat2(1.0,0.0,0.0,100.0) );
    }                                             
    else {
...
static void mainmenu(int id)
{
    ...
    if (id == 63) {
      ...
      for (int i=0; i<nObjects; i++) {
        sceneObjs[i].animationStart += resume - animationPause;
      }
      animationPause = 0; // reset pause animation time
    }
    if (id == 64) {
      if ( nObjects >= maxObjects ) return; // dont add object if no memory for it
      sceneObjs[nObjects] = sceneObjs[currObject];
      toolObj = currObject = nObjects++;
      setToolCallbacks( adjustLocXZ, camRotZ(), adjustScaleY, mat2(0.05,0.0,0.0,10.0) );
      glutPostRedisplay();
    }
    if (id == 65) {
      if( currObject >= 3 ) { // must be more than 3 objects (2 lights + ground + ...scene objs ...)
        
        nObjects--;
        if( nObjects > 3 ) {
          currObject = nObjects - 1;
        } else {
          currObject = -1; // no object currently selected
        }
        toolObj = -1;
...
static void makeMenu()
{
    ...
    glutAddMenuEntry("version 1.0: Ambient/Diffuse/Specular/Shine",20);
    ...
    glutAddMenuEntry("Delete current object", 65);
...
void reshape( int width, int height )
{
    ...
    /* When width is greater or equal than height */
    else {
